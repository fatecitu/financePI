...
export async function carregarLancamentos() {
    ...
    dados.forEach(l => {
        ...
        tr.innerHTML = `          
          ...
          <td class="border p-2 text-right">R$ ${l.valor.toFixed(2)}</td>
          <td class="border p-2 text-right">${new Date(l.data_ocorrencia + 'T00:00:00').toLocaleDateString('pt-BR')}</td>    
          <td class="border p-2 text-right">${l.data_vencimento}</td>
        ...
        // acumula os valores conforme o tipo
        if (l.categorias.tipo === 'receita') {
            totalReceitas += l.valor;
        } else if (l.categorias.tipo === 'despesa') {
  ...
  trTotal.innerHTML = `
    <td colspan="5" class="text-right font-semibold text-gray-700 p-4">Total dos Lançamentos: </td>
    <td class="p-4 font-semibold ${totalMensal >= 0 ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}">
