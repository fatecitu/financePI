...
export async function buscarLancamentos() {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/lancamentos?select=*,categorias(descricao,tipo)`, {
    headers: {
      "apikey": API_KEY,
...
export async function buscarLancamentoPeloId(id) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/lancamentos?id=eq.${id}&select=*,categorias(descricao,tipo)`,
    {
      headers: {
        "apikey": API_KEY,
        "Authorization": `Bearer ${token}`
